 ...
 * @param {String} [options.setDebugLevel] - log4js debug level ('ALL'|'TRACE'|'DEBUG'|'INFO'|...)
 */
function Query(options) {
  this.params = [];

  if (options && options.setDebugLevel) {
    logger.setLevel = options.setDebugLevel;
  }
}
...
Query.prototype.q = function(params) {
  ...
  if (_.isObject(params)) {
    self.params.push(queryPrefix + queryString.stringify(params, '%20AND%20', ':'));
  } else if (_.isString(params)) {
    self.params.push(queryPrefix + encodeURIComponent(params));
  } else {
...
Query.prototype.sort = function(params){
  var self = this;
  self.params.push('sort=' + queryString.stringify(params, ',', ' '));
  return self;
};
