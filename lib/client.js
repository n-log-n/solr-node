 ...
 * @param {Function} finalCallback - (err, result)
 */
Client.prototype.requestPost = function(path, data, urlOptions, finalCallback) {
  var params, options, requestPrefixUrl, requestFullPath;

  if (_.isString(urlOptions)) {
    params = urlOptions;
  } else if (_.isObject(urlOptions)) {
    params = queryString.stringify(urlOptions) + '&wt=json';
  } else {
    params = '';
  }
  ...
  logger.debug('[requestPost] requestFullPath: ', requestFullPath);
  logger.debug('[requestPost] data: ', data);
...
Client.prototype.mlt = function(query, finalCallback) {
  this.requestGet(this.MLT_PATH, query, finalCallback);
};
...
Client.prototype.spell = function(query, finalCallback) {
  this.requestGet(this.SPELL_PATH, query, finalCallback);
};
...
Client.prototype.update = function(data, options, finalCallback) {
  ...
  this.requestPost(this.UPDATE_PATH, bodyData, options, finalCallback);
};
...
Client.prototype.ping = function(finalCallback) {
  this.requestGet(this.PING_PATH, '', finalCallback);
};
...
Client.prototype.commit = function(finalCallback) {
  this.requestPost(this.UPDATE_PATH, {}, {commit: true}, finalCallback);
};
...
Client.prototype.softCommit = function(finalCallback) {
  this.requestPost(this.UPDATE_PATH, {}, {softCommit: true}, finalCallback);
};
